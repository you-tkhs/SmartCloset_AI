...
> **注記**: 本書に記録されたcommit hashは実装当時のものであり、リポジトリ公開に伴う履歴整理(git filter-repoによる機密情報除去、2026-08)により実際の値とは異なる場合がある。ハッシュは「その時点でこの変更が行われた」という作業記録として保持しており、`git show`等でそのまま参照できることは保証しない。
