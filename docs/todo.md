...
- **実装内容**: (1)システムプロンプトの文言を「ユーザーの要望(用途・シーン)を最優先しつつ天気にも配慮した」に変更 (2)ユーザープロンプトテンプレートのセクション順を「ユーザーの要望」→「天気情報」に入れ替え (3)ルールに「request_textから用途・シーンを読み取りTPOに合った選択を最優先で反映する」を追加 (4)既存の天気言及ルールを「シーンに合わせた提案理由を主軸にしつつ天気にも簡潔に触れる」に弱める(削除はしない。天気軽視の再発防止)
- **完了条件**: `backend/tests/test_suggest.py`の新規テストが新ルール文言の反映・セクション順・実際にLLMへ送るmessages[1]内容への反映を検証してパスすること。`suggest_prompt.py`と`docs/design.md`付録B.2が一字一句同一であることを目視diffで確認。ローカルで実OpenAI APIを使い、面接・デート等のリクエストで提案文がシーンに触れることを確認
- **検証コマンド**: `cd backend && python -m pytest -m "not yolo" -q`
- **想定される正常結果**: 既存全件+新規テストがすべてpass
- **想定される異常結果**: 新ルール文言のtypoや`_USER_PROMPT_TEMPLATE`のプレースホルダ名不一致による`KeyError`/`str.format`失敗 → 新規ユニットテスト(`build_suggest_user_prompt`直接呼び出し)で検出
- **推奨コミットメッセージ**: `docs(design): コーデ提案プロンプトの用途・シーン優先ルールを追加`(設計変更) / `feat(suggest): コーデ提案プロンプトで用途・シーン(TPO)を天気より優先するよう調整`(実装)
- **チェック**: 実装済み [x] / テスト済み [x] / commit済み [x] / push済み [ ]
- **commit hash**: `9d930a3`(設計変更commit: `f5b985d`)
- **備考**: `cd backend && python -m pytest -m "not yolo" -q` で148 passed(既存146件+新規2件。1件`test_no_bulk_file_read_in_storage_service`が失敗するが、変更前のコード(mainブランチ)でも同じくWindows環境のcp932デコードエラーで失敗することを確認済みのため今回の変更とは無関係)。ローカルで実OpenAI APIキーを使い、`build_suggest_user_prompt`が生成する実プロンプトに対してLLMを直接呼び出し、3パターンで目視確認: ①「明日の面接に着ていく服を提案してください」→「面接なら〜清潔感のある王道配色で好印象です」とフォーマル・清潔感を主軸に理由づけ ②「週末デートに着ていく服を提案してください」→「週末デートは清潔感重視で〜大人っぽく」と面接とは異なるトーンで理由づけ ③「明日沖縄でデートなので服を提案してください」(天気情報あり)→「沖縄デートは暑さ対策で〜晴れで蒸し暑いので、トップスは通気性重視で選びました」とシーンを主軸にしつつ天気が自然に一文へ織り込まれ、suggestion_textは88字で200字制限内。天気を完全に無視する回帰は確認されず、シーンへの言及が明確に増えたことを確認
