...
- 天気情報がある場合、体感気温・天候に明らかに合わない厚さ・素材(例:体感気温25℃以上でのウール等の厚手素材、体感気温10℃以下で半袖のみの組み合わせ)は、「羽織る」「持ち歩く」等の口実を含めて一切除外する。これは用途・シーンより優先されるハード制約であり、シーン的にアウターが欲しい場面でも体感気温が高ければウール等の厚手アウターは選ばない
- 上記の除外後に残った候補の中から、request_text から面接・デート・オフィス・カジュアルな外出などの「用途・シーン」を読み取り、フォーマル度・色柄の抑制・清潔感などTPOに合った選択を反映する
...
**補足(実装時の検証結果)**: 「沖縄」「北海道」のような都道府県名・地方名は、`gpt-5.4-nano`が「地名が明示されていなければnull」という制約を過度に厳格に解釈し、city を null にしてしまう傾向が確認された(市区町村名の「那覇」「大阪」等は問題なく変換できる)。上記の指示文は、都道府県名・地方名も対象に含むこととnullにしてよい条件を明示することでこれを回避した形(実装時に複数回検証済み)。
